...
//CommonJS syntax module Using node 
// const per = require('./mymodule1')
// ES2015 syntax thes are using into the angular
// import {person1} from './module2' 
// import {sayHello} from './module2'
// We can also write the above statement as ES06 Destruction
// import {person1, sayHello} from './module2'
// import * as mod from './module2'
// import greeting from './module2'
// console.log(mod.person1.name)
// console.log(mod.sayHello())
// console.log(mod)
// console.log(mod.perArr[2])
...
document.querySelector('#posts').addEventListener('click', clearPost);

function getPosts() {

  _http.http.get('http://localhost:3000/post').then(function (data) {
    return _ui.ui.showItem(data);
  }).catch(function (err) {
...
function addPost(e) {
  ...
  var data = {
    ...
    _ui.ui.showAlert('Please write Your Post', 'alert alert-danger');
  } else {
    ...
    if (id === '') {
      // Create Post
      _http.http.post('http://localhost:3000/post', data).then(function (data) {
      ...
      _http.http.put('http://localhost:3000/post/' + id, data).then(function (data) {
        _ui.ui.showAlert('Post Update', 'alert alert-success');
        _ui.ui.formChange('add');
        getPosts();
      }).catch(function (err) {
        return console.log(err);
      });
...
function editPost(e) {
  ...
  if (e.target.parentElement.classList.contains('edit')) {
    ...
    var body = e.target.parentElement.previousElementSibling;
    var title = e.target.parentElement.previousElementSibling.previousElementSibling;
    var data = {
      id: id,
      title: title,
      body: body
...
function clearPost(e) {
  if (confirm('Are you Sure')) {
    if (e.target.parentElement.classList.contains('delete')) {
      var id = e.target.parentElement.dataset.id;
      console.log(id);
      _http.http.delete('http://localhost:3000/post/' + id).then(function (data) {
...
var easyhttp3 = function () {
  function easyhttp3() {
    _classCallCheck(this, easyhttp3);
  }

  _createClass(easyhttp3, [{
    key: 'get',
    value: function () {
...
var http = exports.http = new easyhttp3();
...
var UI = function () {
  ...
  _createClass(UI, [{
    key: 'showItem',
    value: function showItem(post) {
      console.log(post);

      var outPut = '';
      post.forEach(function (element) {
        outPut += '<div class="card mb-3">\n  <div class="card-body">\n  <h4 class="card-title">' + element.title + '</h4>\n  <p class="card-text">' + element.body + '</p>\n  <a href="#" class="edit card-link" data-id="' + element.id + '"><i class="fas fa-pencil-alt"></i></a>\n  <a href="#" class="delete card-link" data-id="' + element.id + '"><i class="fas fa-trash-alt" style\n  ="color:red;"></i></a>\n  </div>\n  \n  </div>';
      });
      this.post.innerHTML = outPut;
    }
  }, {
    key: 'showAlert',
    value: function showAlert(message, clam) {
      var _this = this;
      ...
      document.querySelector('.post-container').insertBefore(div, document.querySelector('#posts'));
    ...
    value: function clearAlert() {
      var current = document.querySelector('.alert');
      if (current) {
        current.remove();
      }
    }
    ...
    value: function fillForm(data) {
      this.title.value = data.title.textContent;
      this.message.value = data.body.textContent;
      this.id.value = data.id;
      console.log(this.id.value);
      this.formChange('edit');
    }
  }, {
    key: 'clearIdInput',
    ...
    key: 'formChange',
    value: function formChange(type) {
      if (type === 'edit') {
        this.btn.textContent = 'Update Post';
        this.btn.className = 'post-edit btn btn-danger btn-block';
        var btn = document.createElement('button');
        btn.className = 'post-cancel btn btn-secondary btn-block';
        btn.appendChild(document.createTextNode('Cancel Edit'));
        var parent = document.querySelector('.card-form');;
        var end = document.querySelector('.form-end');
        parent.insertBefore(btn, end);
      } else {
        this.btn.textContent = 'PostIt';
        this.btn.className = 'post-submit btn btn-primary btn-block';
